...
async function startServer() {
  ...
  // Helper for simulated active user header / query
  ...
  // -------------------------------------------------------------
  // APPLICATIONS API & STATE MACHINE
  // -------------------------------------------------------------
  ...
  app.get("/api/employer/applications", async (req, res) => {
    try {
      ...
      // Enrich applications with candidate profile and resume document info
  ...
  // Employer: Internal Notes update
  ...
  // -------------------------------------------------------------
  // ADMIN MODERATION & FLAG REPORTS
  // -------------------------------------------------------------
  app.get("/api/admin/companies", async (req, res) => {
    try {
      const compList = await pgGetCompanies();
      res.json(compList);
    } catch (err) {
  ...
  app.post("/api/reports", async (req, res) => {
    try {
      ...
      const newReport = await pgCreateFlagReport({
        ...
        details: details || "",
        status: "open",
        reported_by_user_id: user ? user.id : "anonymous",
        reported_by_name: user ? user.name : "Anonymous User",
        created_at: new Date().toISOString()
      });
  ...
  app.post("/api/admin/reset-seed", (req, res) => {
    res.json({ success: true, message: "Seed data active and pristine on Cloud SQL." });
  });
